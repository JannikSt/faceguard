...
export default function Start() {
  ...
  const submit = (imgObject1: any) => {
    ...
    reqwest({
      ...
      success: (res: any) => {
        ...
        if (res.isMatching) message.success(`Login successfull!`);
      },
      error: () => {
        setSubmitLoading(false);
  ...
  const handleCancel = () => {
    setIsLoginModalVisisble(false);
    setIsWebCamVisible(false);
    // success not req bc this is the new spotify foto
  ...
  return (
    <div>
      ...
      <div>
        <img
          className="spotifyFake"
          src={require('../../assets/FaceGuardSpotify.png')}
          onClick={() => setIsLoginModalVisisble(true)}
          alt=""
        />
      </div>
      {showSuccess && (
        <div>
          <img
            alt=""
            className="spotifyFake"
            src={require('../../assets/SpotifyAfterLogin.png')}
            //onClick={() => setIsLoginModalVisisble(true)}
          />
        </div>
      )}
